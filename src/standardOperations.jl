...
hyperbolic = FlagButton(:hyperbolic, "Hyp")
...
macro family(arity, description)
    if description.head != :vcat || length(description.args) != 4
        error("Format @family arity, [default ; inv ; hyp ; inv hyp] where rows are either 
'function [name [description]]' or '(symbol, function) [name [description]]' and if arity is 0, function is an operation");
    ...
    function od(odsym, tup)
        (sym, x, n, d) = tup
        exp = isa(x, Symbol) ? :(Base.Math.$x) : x
        if arity == 0
            :($odsym = OperationDescription($n, $d, $exp))
        else
            :($odsym = OperationDescription($n, $d, StandardOperation($exp, $arity)))
        end
    end
    function ops(indices)
        if isa(rows[indices[1]][1], Symbol)
            :(global $(rows[indices[1]][1]) = Multibutton(
    ...
    Expr(:let, defns, od.(ods, rows)...)
end
i(x) = x # we use this to stop @family from treating symbols in a special way

# constants
export pi, e, phi, eulergamma
@family 0 [(pi,    InsertionOperation(Base.Math.π)) "π" "the constant π=3.14159..."
...
@family 1 [exp "exp" "the exponential function, e^(1:)"
...
@family 1 [sqrt "sqrt" "square root"
           (x -> x*x) "^2" "square"
           cbrt "cbrt" "cube root"
           (x -> x*x*x) "^3" "cube" ]
...
export abs, inv
abs = Button(OperationDescription("abs", "the magnitude of (1:)", StandardOperation(Base.abs, 1)))
...
backspace = Button(OperationDescription("⌫", "backspace", BackspaceOperation()))
enter = Button(OperationDescription("Enter", "", EnterOperation()))
...
factorial = Multibutton(() => factorialo, (:inverse,) => factorialo, (:hyperbolic,) => doublefactorialo, (:inverse,:hyperbolic) => doublefactorialo)
